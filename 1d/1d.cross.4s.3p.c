...
void cross_1d4s3p(double **A, double **R, int N, int T, int Bx, int tb)
{
	...
	for (tt = -tb; tt < T; tt += tb)
	{
		...
		{
			vec v1, v2, v3, v4, v5,v6,v7,v8,v9,v10,v11,v12,v13,v14;
			vec post1, post2;
			for (t = max(tt, 0); t < min(tt + 2 * tb, T); t+=4)
			{
				...
				xmod = (xmax - xmin) % veclen6;
				xup = xmax - xmod;

				for (x = xmin; x < xup; x +=veclen6)
				{
					if (x == xmin)
					{
						setload(v1, v2, v3, v4, B[(t/4)  % 2][x - 1]);
						setload(v5, v6, v7, v8, B[(t/4)  % 2][x + veclen4]);
						setload(v9,v10,v11,v12, B[(t/4)  % 2][x + 2*veclen4]);
						vload(v13, B[(t/4)  % 2][x + 3*veclen4]);
						vload(v14,B[(t/4)  % 2][x + 3*veclen4+veclen]);
					}
					else
					{
						setload(v9,v10,v11,v12,B[(t/4)  % 2][x+2*veclen4]);
					...
					cross_cp_s4p3(v1, v2, v3, v4, v5, v6, v7, v8, v9, ww);
					cross_cp_s4p3(v2, v3, v4, v5, v6, v7, v8, v9,v10, ww);
					cross_cp_s4p3(v3, v4, v5, v6, v7, v8, v9, v10,v11,ww);
					cross_cp_s4p3(v4, v5, v6, v7, v8, v9, v10, v11,v12,ww);
					cross_cp_s4p3(v5, v6, v7, v8, v9, v10,v11,v12,v13,ww);
					cross_cp_s4p3(v6, v7, v8, v9, v10, v11,v12,v13,v14,ww);

					setstore(v1,v2,v3,v4,B[(t/4 + 1) % 2][x+veclen4]);
					vstore(B[(t/4 + 1) % 2][x+2*veclen4],v5);
					vstore(B[(t/4 + 1) % 2][x+2*veclen4+veclen],v6);
	...
	for (i = 0; i < num_row; i++) {
		for (j = 0; j < num_per_row; j++) {
			B[(T + 1) % 2][num_row * j + i + veclen] = B[(t/4)  % 2][num_per_row * i + j + veclen];
		}
	}
	gettimeofday(&end, 0);
	printf("cross_1d4s3p GStencil/s = %f\n", ((double)N * T) / (double)(end.tv_sec - start.tv_sec + (end.tv_usec - start.tv_usec) * 1.0e-6) / 1000000000L);

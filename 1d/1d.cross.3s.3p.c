...
void cross_1d3s3p(double **A, double **R, int N, int T, int Bx, int tb)
{
	...
	for (tt = -tb; tt < T; tt += tb)
	{
		...
		for (xx = 0; xx < nb0[level]; xx++)
		{
			...
			vec post1, post2;
			for (t = max(tt, 0); t < min(tt + 2 * tb, T); t+=3)
			{
				...
				for (x = xmin; x < xup; x +=veclen4)
				{
					if (x == xmin)
					{
						setload(v1, v2, v3, v4, B[(t/3)  % 2][x - 1]);
						setload(v5, v6, v7, v8, B[(t/3)  % 2][x + veclen4]);
						vload(v9, B[(t/3)  % 2][x + 2*veclen4]);
						vload(v10,B[(t/3)  % 2][x + 2*veclen4+veclen]);
					}
					else
					{
						setload(v7, v8, v9,v10,B[(t/3)  % 2][x+2*veclen4])
					}

					// results are stored in v1 v2 v3 v4
					cross_cp_s3p3(v1, v2, v3, v4, v5, v6, v7, ww);
					cross_cp_s3p3(v2, v3, v4, v5, v6, v7, v8, ww);
					cross_cp_s3p3(v3, v4, v5, v6, v7, v8, v9, ww);
					cross_cp_s3p3(v4, v5, v6, v7, v8, v9, v10, ww);

					setstore(v1,v2,v3,v4,B[(t/3 + 1) % 2][x+veclen3]);
	...
	for (i = 0; i < num_row; i++) {
		for (j = 0; j < num_per_row; j++) {
			B[(T + 1) % 2][num_row * j + i + veclen] = B[(t/3)  % 2][num_per_row * i + j + veclen];
		}
	}
	gettimeofday(&end, 0);
	printf("cross_1d3s3p GStencil/s = %f\n", ((double)N * T) / (double)(end.tv_sec - start.tv_sec + (end.tv_usec - start.tv_usec) * 1.0e-6) / 1000000000L);

...
void fold_1d4s5p(double **A, double **R, int N, int T, int Bx, int tb)
{
	...
	for (tt = -tb; tt < T; tt += tb)
	{
		...
		for (xx = 0; xx < nb0[level]; xx++)
		{
			...
			vec post1, post2;
			for (t = max(tt, 0); t < min(tt + 2 * tb, T); t+=6)
			{
				...
				for (x = xmin; x < xup; x +=veclen4)
				{
					...
					fold_cp_s4p5(v1, v2, v3, v4, v5,v9);
					fold_cp_s4p5(v2, v3, v4, v5, v6,v10);
					fold_cp_s4p5(v3, v4, v5, v6, v7,v11);
					fold_cp_s4p5(v4, v5, v6, v7, v8,v12);
					shuffle_head(v2,v3,v2);
					shuffle_head(v3,v4,v3);
					shuffle_head(v4,v5,v4);
					shuffle_head(v5,v6,v5);
					shuffle_head(v6,v7,v6);
					shuffle_head(v7,v8,v7);
					shuffle_head(v8,v9,v8);
					fold_cp_s4p5n2( v2, v3, v4, v5,v9);
					fold_cp_s4p5n2( v3, v4, v5, v6,v10);
					fold_cp_s4p5n2( v4, v5, v6, v7,v11);
					fold_cp_s4p5n2( v5, v6, v7, v8,v12);
					shuffle_head(v2,v3,v2);
					shuffle_head(v3,v4,v3);
					shuffle_head(v4,v5,v4);
					shuffle_head(v5,v6,v5);
					...
					setstore(v9,v10,v11,v12,B[(t/2 + 1) % 2][x+veclen2]);

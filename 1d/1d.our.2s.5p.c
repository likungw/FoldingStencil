...
void our_1d2s5p(double** A, double** R, int N, int T, int Bx, int tb){
	...
	for (tt = -tb; tt < T ;  tt += tb ){
		#pragma omp parallel for private(xmin,xmax,t,x,xx,xe,ww,kk)
				...
				if((tt == -tb)||(t >= tt + tb)){					
					...
					for(x = xmin; x < xe; x+=veclen4){
						if(x == xmin){					
							...
							setload(v5,v6,v7,v8,B[(t/2)%2][x-1+veclen]);
							trans(v1, v2, v3, v4);
						}
						else{
							setload(v5,v6,v7,v8,B[(t/2)%2][x-1+veclen]);
						...
						//post2 = tail2(v2,v5);
						shuffle_tail(v2,v6,post2);
						shuffle_tail(v2,v7,post3);
						shuffle_tail(v2,v8,post4);
						setcompute_5p(v1,v2,v3,v4,post1,post2,post3,post4,ww,kk); 
						if(x == xmin){
							if(x == 1){
								vec tmp = v1 + v2;
							...
							setcompute_5p(w1,w2,w3,w4,post1,post2,post3,post4,ww,kk);							
							//trans(w1,w2,w3,w4);	
							setstore(w1,w2,w3,w4,B[((t/2)+1)%2][x+1-veclen4]);
						}
						w1 = v1;
						w2 = v2;
						w3 = v3;
					...
					for(x = xmin; x < xe; x+=veclen4){
						if(x == xmin){					
							...
							setload(v5,v6,v7,v8,B[(t/2)%2][x-3+veclen]);
							trans(v1, v2, v3, v4);
							pre2 = v0;
							pre1 = v0;
						}
						else{
							setload(v5,v6,v7,v8,B[(t/2)%2][x+1]);
						}	
						if(tt==-tb)					
						trans(v5,v6,v7,v8);						
						//pre1 = head1(v4,pre1);
						...
						if(x == xmin ){
							//pre2 = head2(v3,pre2);
							shuffle_head2(v3,pre2,pre2);
						}
						else{
							//pre2 = head1(v3,pre2);
							shuffle_head(v3,pre2,pre2);
						}		
						shuffle_head(v3,v7,pre3);
						shuffle_head(v4,v8,pre4);
						setcompute_5p(pre4,pre3,pre2,pre1,v1,v2,v3,v4,ww,kk);
						if(x == xmin ){
						...
						shuffle_head(v1,v5,pre1);
						shuffle_head(v2,v6,pre2);
						setcompute_5p(pre4,pre3,pre2,pre1,v1,v2,v3,v4,ww,kk);
						if(x == 1){
							//pre4 = boundary(pre4);

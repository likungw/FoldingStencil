...
#pragma omp parallel for schedule(dynamic) private(xmin, xmax, ymin, ymax, n, t, x, y, z) firstprivate(ww, kk, zmod, zup)
		for (n = 0; n < nb02[level]; n++)
		{
			...
			vec vc_h1, vc_h2, vc_h3, vc_h4, vl_1, vr_1, vt_1, vb_1, vu, vd;
					...
					{						
						for (z = ZSLOPE; z < zup; z += veclen4)
						{
							...
							{	
								...
								vload(vl_1, C[t % 2][x][y - 1][z]);
								vload(vl_2, C[t % 2][x][y - 1][z+veclen]);
								vl_1_s2 = vl_1;
								vr_1_s2 = vr_1;
							} 

							// load tops
							vload(vt_2, C[t % 2][x][y - 1][z+veclen2]);
							vload(vt_1, C[t % 2][x][y - 1][z+veclen3]);

							//load bottoms						
							vload(vb_1, C[t % 2][x][y + 1][z]);
							vload(vb_2, C[t % 2][x][y + 1][z+veclen]);

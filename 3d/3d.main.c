...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < 2; i++)
	{
		...
		Y[i] = (double ***)malloc(sizeof(double **) * (NX + 2 * XSLOPE));
		Z[i] = (double ***)malloc(sizeof(double **) * (NX + 2 * XSLOPE));
	...
	for (i = 0; i < 2; i++)
	{
		for (j = 0; j < (NX + 2 * XSLOPE); j++)
		{
			...
			Y[i][j] = (double **)malloc(sizeof(double *) * (NY + 2 * YSLOPE));
			Z[i][j] = (double **)malloc(sizeof(double *) * (NY + 2 * YSLOPE));
	...
	for (i = 0; i < 2; i++)
	{
		for (j = 0; j < (NX + 2 * XSLOPE); j++)
		{
			for (k = 0; k < (NY + 2 * YSLOPE); k++)
			{
				...
				Y[i][j][k] = (double *)malloc(sizeof(double) * (NZ + 2 * ZSLOPE));
				Z[i][j][k] = (double *)malloc(sizeof(double) * (NZ + 2 * ZSLOPE));
	...
	for (i = 0; i < NX + 2 * XSLOPE; i++)
	{
		for (j = 0; j < NY + 2 * YSLOPE; j++)
		{
			for (k = 0; k < NZ + 2 * ZSLOPE; k++)
			{
				...
				// A[0][i][j][k] = i * 2 + j / 2 + k;
				A[1][i][j][k] = 0;
				Y[0][i][j][k] = 1.0 * (rand() % 1024);
				// A[0][i][j][k] = i * 2 + j / 2 + k;
				Y[1][i][j][k] = 0;
				Z[0][i][j][k] = 1.0 * (rand() % 1024);
				// A[0][i][j][k] = i * 2 + j / 2 + k;
				Z[1][i][j][k] = 0;
				B[0][i][j][k] = A[0][i][j][k];
				B[1][i][j][k] = 0;

...
void cross_2d1s5p(double ***A, int NX, int NY, int T, int Bx, int By, int tb)
{
	...
	vec ww, kk, gw;
	vec vc_h1, vc_h2, vc_h3, vc_h4, vl, vr, vt, vb;
			...
			for (t = max(tt, 0); t < min(tt + 2 * tb, T); t++)
			{
				...
				for (x = xmin; x < xmax; x ++)
				{
					for (y = ymin; y < yup; y += veclen4)
					{
						...
						//vertical comp.
						cross_comp_h6(vt, vc_h1, vc_h2, vc_h3, vc_h4, vb);
						...
						//store
						setstore(vt, vc_h1, vc_h2, vc_h3, C[(t + 1) % 2][x][y]);

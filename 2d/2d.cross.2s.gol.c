...
void cross_2d2sgol(double ***A, int NX, int NY, int T, int Bx, int By, int tb)
{
	...
	vec vctmp_1,vctmp_2, vctmp_3,vctmp_4;
	vec vntmp_1,vntmp_2, vntmp_3,vntmp_4;
			...
			for (t = max(tt, 0); t < min(tt + 2 * tb, T); t+=2)
			{
				...
				for (x = xmin+1; x < xmax-1; x++)
				{
					...
					{						
						if (y == ymin)
						{							
							setload(vc_h1, vc_h2, vc_h3, vc_h4, C[((t/2) % 2)][x][y]);
							//vloadset(vl, C[((t/2) % 2)][x][y], C[((t/2) % 2)][x + 1][y], C[((t/2) % 2)][x + 2][y], C[((t/2) % 2)][x + 3][y]);
							vload(vl_1, C[((t/2) % 2)][x - 1][y]);
							vload(vl_2, C[((t/2) % 2)][x - 1][y+veclen]);
							vl_1_s2 = vl_1;
							vr_1_s2 = vr_1;
						} 

						// load top
						vload(vt_2, C[((t/2) % 2)][x - 1][y+veclen2]);
						vload(vt_1, C[((t/2) % 2)][x - 1][y+veclen3]);

						//load next
						setload(vn_h1, vn_h2, vn_h3, vn_h4, C[((t/2) % 2)][x][y+veclen4]);

						//load bottom						
						vload(vb_1, C[((t/2) % 2)][x + 1][y]);
						vload(vb_2, C[((t/2) % 2)][x + 1][y+veclen]);

						//2-nb vertical comp.
						cross_comp_s2p9_ma1(vn_v1, vn_v2, vn_v3, vn_v4, vt_2, vt_1, vn_h1, vn_h2, vn_h3, vn_h4, vb_2, vb_1);
						cross_comp_s2p9_ma2(vn_v1, vn_v2, vn_v3, vn_v4, vt_2, vt_1, vn_h1, vn_h2, vn_h3, vn_h4, vb_2, vb_1);
						...
						//next vertical comp.
						cross_comp_h6(vt_1, vntmp_1, vntmp_2, vntmp_3, vntmp_4, vb_1);

						//transpose next
						trans(vn_v1, vn_v2, vn_v3, vn_v4);

						//2-nb horizontal comp.
						cross_comp_gol2nb(vc_v1, vc_v2, vc_v3, vc_v4, vl_2, vl_1, vc_v1, vc_v2, vc_v3, vc_v4, vn_v1, vn_v2);

						//transpose next
						trans(vt_1, vntmp_1, vntmp_2, vntmp_3);

						cross_comp_h6(vt_1, vctmp_1, vctmp_2, vctmp_3, vctmp_4,vt_1)

						cross_comp_s2_nb2_v8(vt_1, vctmp_1, vctmp_2, vctmp_3, vc_v1, vc_v2, vc_v3, vc_v4);
						...
						//results
						cross_comp_s2p9_m4(vc_v1, vc_v2, vc_v3, vc_v4, kk);
						...
						//store
						setstore(vc_h1, vc_h2, vc_h3, vc_h4, C[(t + 1) % 2][x][y]);
